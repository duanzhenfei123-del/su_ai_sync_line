...
function Assert-PayloadMatchesSource {
    ...
    try {
        ...
        if ($sourceFiles.Count -ne 12 -or $entries.Count -ne 12 -or $mismatches.Count -ne 0) {
            throw "Payload mismatch: source=$($sourceFiles.Count), entries=$($entries.Count), errors=$($mismatches -join '; ')"
        }
    }
    finally {
        $sha.Dispose()
...
if ($version.FileVersion -ne '3.7.0.0' -or $version.ProductVersion -notlike '3.7*') {
