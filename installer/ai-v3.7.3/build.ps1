...
$buildDir = [IO.Path]::GetFullPath((Join-Path $repoRoot 'build\ai-installer-v3.7.3'))
$payload = Join-Path $buildDir 'PluginPayload.zip'
$testExe = Join-Path $buildDir 'InstallerCoreTests.exe'
$releaseDir = [IO.Path]::GetFullPath((Join-Path $repoRoot 'releases\v3.7.3'))
$zipOutput = Join-Path $releaseDir 'SU_AI_PNG_Export_v3.7.3.zip'
$output = Join-Path $releaseDir 'SU_AI_PNG_Export_v3.7.3_Setup.exe'
...
if ($version.FileVersion -ne '3.7.3.0' -or $version.ProductVersion -ne '3.7.3') {
