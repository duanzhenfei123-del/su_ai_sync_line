...
$buildDir = [IO.Path]::GetFullPath((Join-Path $repoRoot 'build\ai-installer-v3.7.2'))
$payload = Join-Path $buildDir 'PluginPayload.zip'
$testExe = Join-Path $buildDir 'InstallerCoreTests.exe'
$releaseDir = [IO.Path]::GetFullPath((Join-Path $repoRoot 'releases\v3.7.2'))
$zipOutput = Join-Path $releaseDir 'SU_AI_PNG_Export_v3.7.2.zip'
$output = Join-Path $releaseDir 'SU_AI_PNG_Export_v3.7.2_Setup.exe'
...
if (-not $releaseDir.StartsWith(([IO.Path]::GetFullPath((Join-Path $repoRoot 'releases')) + [IO.Path]::DirectorySeparatorChar), [StringComparison]::OrdinalIgnoreCase)) {
...
if ($version.FileVersion -ne '3.7.2.0' -or $version.ProductVersion -ne '3.7.2') {
